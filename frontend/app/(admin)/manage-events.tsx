...
export default function ManageEvents() {
  ...
  const fetchEvents = async () => {
    try {
      setLoading(true);
      const response = await api.get('/events?approvalStatus=all');
      // Filter out past events
      const now = new Date();
      const futureEvents = (response.data.data || response.data).filter((event: Event) => 
  ...
  const handleDelete = (eventId: string, eventTitle: string) => {
    Alert.alert(
      ...
      [
        ...
        { text: 'Delete', style: 'destructive', onPress: () => performDelete(eventId) }
      ]
    );
  };
  ...
  const handleToggleStatus = async (eventId: string, currentStatus: boolean) => {
    ...
    try {
      await api.patch(`/events/${eventId}/status`, { isActive: newStatus });
      setEvents(events.map(event => 
        event.id === eventId ? { ...event, isActive: newStatus } : event
      ));
      Alert.alert('Success', `Event ${action}d successfully`);
    } catch (error) {
  ...
  const getTabCount = (tab: TabType) => {
    ...
    if (tab === 'active') return events.filter(e => e.isActive).length;
    if (tab === 'inactive') return events.filter(e => !e.isActive).length;
    return 0;
  };
          ...
          <View style={styles.header}>
            <View>
              ...
              <Text style={styles.subtitle}>
                {filteredEvents.length} events • Page {currentPage} of {totalPages || 1}
              </Text>
            </View>
            <TouchableOpacity 
                  ...
                  return (
                    ...
                    >
                      ...
                      <View style={[styles.tableCell, styles.typeColumn]}>
                        <Text style={styles.tableCellText}>{event.eventType}</Text>
                      </View>
