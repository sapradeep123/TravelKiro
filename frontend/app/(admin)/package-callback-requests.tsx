...
export default function PackageCallbackRequestsScreen() {
  const [requests, setRequests] = useState<CallbackRequest[]>([]);
  const [event, setEvent] = useState<Package | null>(null);
  ...
  const loadData = async () => {
    ...
    try {
      setLoading(true);
      console.log('Loading data for package:', packageId);
      
      // Load Package details
      const eventData = await packageService.getEventById(packageId);
      console.log('Package data loaded:', eventData);
      setEvent(eventData);
      
      // Load callback requests for this event
      const requestsData = await packageService.getEventCallbackRequests(packageId);
  ...
  const handleMarkContacted = async (requestId: string) => {
    try {
      await packageService.markAsContacted(requestId);
          ...
          <TouchableOpacity 
            ...
            onPress={() => {
              if (Platform.OS === 'web' && typeof window !== 'undefined') {
                ...
                router.push('/(admin)/manage-packages' as any);
              }
            }}
          >
