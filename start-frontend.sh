...
# serve -s enables SPA mode (serves index.html for all routes)
if command -v serve &> /dev/null; then
    nohup serve -s . -l 8082 > /tmp/frontend-server.log 2>&1 &
elif command -v npx &> /dev/null; then
    nohup npx -y serve -s . -l 8082 > /tmp/frontend-server.log 2>&1 &
else
